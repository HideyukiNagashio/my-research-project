...
def get_phase_indices(seq_len=200, seq_start_pct=0.0, seq_end_pct=100.0):
...
def plot_dynamics_map(dynamics_map, out_label, in_label, x_tick_indices, x_tick_labels, y_tick_indices, y_tick_labels, stride_type_out, stride_type_in, save_path=None, timers=None):
    ...
    if args.exp_dir is None:
        ...
        for o_c in demo_out_cols:
            for i_c in demo_in_cols:
                ...
                plot_dynamics_map(dynamics_map, target_names[o_c], feature_names[i_c], x_tick_indices, x_tick_labels, y_tick_indices, y_tick_labels, stride_type_out=stride_type_Y, stride_type_in=stride_type_X, save_path=save_path)
        ...
        for o_c in demo_out_cols:
            ...
            plot_overall_average_map(mean_map, target_names[o_c], feature_names, y_tick_indices, y_tick_labels, stride_type_X, save_path=save_path)
            print(f"Saved: {save_path}")
            
        # Approach 3: Phase
        print("\n--- Approach 3: Computing Phase-wise Smoothed Maps (Demo) ---")
        for o_c in demo_out_cols:
            phase_maps = compute_phase_smoothed_maps(model, input_data, out_col=o_c, seq_start_pct=start_pct_out, seq_end_pct=end_pct_out)
