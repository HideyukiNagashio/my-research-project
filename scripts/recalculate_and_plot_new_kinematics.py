...
MASSES       = [60.9, 63.8, 68.7, 65.9, 77.0, 74.0, 63.8, 64.5, 73.2, 53.5, 70.7, 47.9]
CONDITIONS   = ['h', 'm', 'l']
...
_BASE_COLS     = [c.replace('Left_', '') for c in COLS_LEFT]
COLS_IPSI      = _BASE_COLS
COLS_CONTRA    = ['Contra_' + c for c in _BASE_COLS]
COLS_BILATERAL = COLS_IPSI + COLS_CONTRA
...
# ============================================================
def validate_angles(df_angles, participant, condition, local_log, warn_only=True):
    thresholds = {
        'Right_Hip_Flex_Ext':    (-15, 45),
        'Left_Hip_Flex_Ext':     (-15, 45),
        'Right_Hip_Abd_Add':     (-20, 20),
        'Left_Hip_Abd_Add':      (-20, 20),
        'Right_Knee_Flex_Ext':   (  0, 75),
        'Left_Knee_Flex_Ext':    (  0, 75),
        'Right_Ankle_Flex_Ext':  (-25, 20),
        'Left_Ankle_Flex_Ext':   (-25, 20),
    }
    issues = {}
    for col, (lo, hi) in thresholds.items():
        if col not in df_angles.columns:
            continue
        mask = (df_angles[col] < lo) | (df_angles[col] > hi)
        if mask.any():
            issues[col] = {
                'n_frames':  int(mask.sum()),
                'fraction':  float(mask.mean()),
                'range':     (float(df_angles[col].min()),
                              float(df_angles[col].max())),
            }
            local_log.append({
                ...
                'n_frames': int(mask.sum()),
                'fraction': float(mask.mean()),
                'min': float(df_angles[col].min()),
                'max': float(df_angles[col].max())
            })
    if issues:
        for col, info in issues.items():
            msg = (f"[WARNING] {participant}_{condition} - {col}: {info['n_frames']} frames ({info['fraction']*100:.1f}%)"
                   f" out of range {info['range']}")
            if not warn_only:
                raise ValueError(msg)
    return issues

def validate_angle_velocity(df_angles, participant, condition, dt, local_log, max_omega_deg_s=500.0):
    angle_cols = [c for c in df_angles.columns if c != 'Time (Seconds)']
    deriv = df_angles[angle_cols].diff().abs() / dt
    mask  = (deriv > max_omega_deg_s).any(axis=1)
    if mask.any():
        local_log.append({
            'participant': participant,
            'condition': condition,
            ...
            'fraction': float(mask.mean()),
            'min': 0.0,
            'max': float(deriv.max().max())
        })
        print(f"  [WARNING] {participant}_{condition} - {mask.sum()} frames exceed angular velocity threshold of {max_omega_deg_s}°/s")
    return mask

# ============================================================
# UPDATED KINEMATICS FUNCTIONS
# ============================================================
...
def build_all_segment_cs_batch(pos_3d, side):
    ...
    if side == 'right':
        ...
        p_gt   = pos_3d[:, RGT]; p_gt_c   = pos_3d[:, LGT]
        p_knee = pos_3d[:, RK];  p_mall   = pos_3d[:, RM]
        p_toe  = pos_3d[:, RT]
    else:
        p_asis = pos_3d[:, LI];  p_asis_c = pos_3d[:, RI]
        p_gt   = pos_3d[:, LGT]; p_gt_c   = pos_3d[:, RGT]
        p_knee = pos_3d[:, LK];  p_mall   = pos_3d[:, LM]
        p_toe  = pos_3d[:, LT]

    # Pelvis CS with updated superior axis reference definition
    e_ml        = _normalize_batch(p_asis - p_asis_c)
    p_asis_mid  = (p_asis + p_asis_c) / 2.0
    p_gt_mid    = (p_gt   + p_gt_c)   / 2.0
    e_sup_ref   = _normalize_batch(p_asis_mid - p_gt_mid)
    R_pelvis    = _build_cs_long_axis_batch(e_sup_ref, e_ml)
...
def process_mocap_data_target_calibration(df_target, df_ref, participant, condition):
    target_cols = []
    for i in range(1, 11):
        p = f"{i:03}"
    ...
    # Run Validations
    local_log = []
    validate_angles(df_result, participant, condition, local_log)
    validate_angle_velocity(df_result, participant, condition, dt=1.0/MOCAP_FREQ, local_log=local_log)
    
    if 'Time (Seconds)' in df_target.columns:
        df_result.insert(0, 'Time (Seconds)', df_target['Time (Seconds)'].values)
    return df_result, columns, local_log
...
def _process_single(args):
    ...
    try:
        ...
        df_angles, _, local_log = process_mocap_data_target_calibration(df_mc, df_l, participant, condition)
        ...
        print(f"✓ {participant}_{condition}: {merged_ens.shape[0]} strides processed.")
...
def process_all_data(output_dir='data/interim/raw_strides_new', n_workers=None):
    os.makedirs(output_dir, exist_ok=True)
    n_workers = n_workers or min(cpu_count(), 6)
    print(f"\n[Phase 1] Processing raw data with updated Pelvis CS...")
    ...
    print(f"\nProcessed {len(all_results)}/{len(args)} conditions successfully.")
...
# ============================================================
def plot_new_raw_strides(data_dir='data/interim/raw_strides_new', output_dir='outputs/new_raw_strides_plots'):
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Focus on the 9 joint angles (Hip, Knee, Ankle x 3 DoFs)
    plot_columns = [
    ...
    for col_name in plot_columns:
        ...
        fig.suptitle(col_name.replace("_", " ") + " (New Pelvis CS)", fontsize=16, fontweight='bold')
        ...
        save_path = os.path.join(output_dir, f"plot_{col_name}_ave.png")
        fig.savefig(save_path, dpi=150, bbox_inches='tight')
        print(f"  Saved ensemble average plot: {save_path}")
        plt.close(fig)
...
if __name__ == "__main__":
    print("=== Start Recalculation & Validation ===")
    
    # 1. Process All Data with New Kinematics and Validations
    all_results = process_all_data()
    
    # 2. Write Validation Summary to a file
    val_log = []
    for r in all_results:
        if 'validation_log' in r:
            val_log.extend(r['validation_log'])
            
    val_report_path = 'outputs/validation_report.csv'
    df_val = pd.DataFrame(val_log)
    df_val.to_csv(val_report_path, index=False)
    print(f"\nValidation report saved to {val_report_path}")
    
    # Compile a quick summary of major issues
    if not df_val.empty:
        summary = df_val.groupby(['participant', 'type']).agg({
            'n_frames': 'sum',
            'fraction': 'mean',
            'max': 'max'
        }).reset_index()
        ...
        print("\nNo validation issues detected! All angles are within normal ranges.")

    # 3. Generate Average Profile Plots
    print("\n=== Generating Ensemble Plots for Recalculated Angles ===")
    plot_new_raw_strides()
    print("=== Done ===")
