...
class HybridGRFAlignedModel(nn.Module):
    """
    ノーマルTransformerの仕様（固定PE, スケール調整, 2層FC）に
    合わせたGCNハイブリッドモデル。
    """
    def __init__(self, input_dim=14, output_dim=3, use_shortcut=False, seq_len=200, 
        ...
        super(HybridGRFAlignedModel, self).__init__()
    ...
    def forward(self, x):
        ...
        g_out = F.relu(self.conv2(g_out, batched_edge_index, batched_edge_weight))
