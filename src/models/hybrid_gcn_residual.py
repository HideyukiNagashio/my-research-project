...
class HybridGCNResidualModel(nn.Module):
    def __init__(self, input_dim=14, output_dim=3, use_shortcut=False, seq_len=200, 
        ...
        super(HybridGCNResidualModel, self).__init__()
        
        self.gnn_out_dim = gnn_out_dim
        imu_dim = input_dim - 8
        # residual: 8 nodes (pure pressure values) = 8
        residual_dim = 8
        combined_dim = cnn_pool_dim + residual_dim + imu_dim # 32 + 8 + 6 = 46
    ...
    def forward(self, x):
        ...
        # Raw features for residual (B, 200, 8)
        raw_residual = pressure
        ...
        g_out = F.relu(self.conv1(node_features, batched_edge_index, batched_edge_weight)) # shape: (B*200*8, 16)
        g_out = F.relu(self.conv2(g_out, batched_edge_index, batched_edge_weight)) # shape: (B*200*8, 16)
        ...
        # Concat GCN output, raw residual, and IMU
        combined = torch.cat([pooled, raw_residual, imu], dim=2) # shape: (B, 200, 46)
